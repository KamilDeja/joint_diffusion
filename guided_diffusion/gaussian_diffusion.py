...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        self.classifier_loss_scaling = classifier_loss_scaling
    ...
    def calculate_classifier_loss(self, model, x_start, y, step, x_t, t):
        ...
        if self.semi_supervised_training and (
                selected_indices_with_labels.sum() != len(selected_indices_with_labels)):
            if (len(self.semi_supervised_data_buffer) + selected_indices_with_labels.sum()) >= len(
        ...
        if zero_classifier_loss or (step < self.late_classifier_step):
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, step=0):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.train_with_classifier and (not self.skip_classifier_loss):
                if self.train_only_classifier:
                    terms["loss"] *= 0
                y = model_kwargs['y']
                loss_classifier = self.calculate_classifier_loss(model, x_start, y, step, x_t, t)
                terms["loss"] += self.classifier_loss_scaling * loss_classifier
